...
| An executable status-only acceptance gate is compared descriptively against contract detection. | `artifacts/cvm/results/diagnostic_experiment.json`; `artifacts/cvm/results/summary.json`; `tests/test_trace_diagnostics.py`; `tests/test_aggregate_cvm.py`: `test_integration_effectiveness_counts_status_only_route_baseline`. | Covered for 15 designed faults, 15 separately instantiated valid controls, and the semantic route boundary. The gate is not another integration framework. |

## Temporal Contract

| Behavior | Evidence | Status |
|---|---|---|
| Final protocol serialization rejects empty trajectories, point/heading count mismatches, and non-finite coordinates or headings. | `tests/test_challenge_e2e_driver.py`: `test_prediction_to_proto_trajectory_rejects_invalid_outputs`. | Covered before output reaches the simulator protocol. |
| Multiple accepted camera aliases select the freshest frame, using alias priority only for timestamp ties. | `tests/test_challenge_e2e_driver.py`: `test_challenge_adapter_selects_freshest_accepted_camera_alias`. | Covered by a regression test for a defect exposed while constructing clean protocol sessions. |
...
| Public-core entry points are deterministic and optional gated entry points remain discoverable. | `tests/test_alpasim_integration.py`: `test_constant_velocity_baseline_is_dependency_light_and_auditable`, `test_route_following_baseline_tracks_supplied_route_geometry`, `test_direct_actor_planner_returns_trajectory_without_token_selector`; `tests/test_alpabridge_doctor.py`: `test_build_report_validates_public_release_surface`. | Covered: constant-velocity and route-following are public core; direct actor-aware planning is an optional gated extension. |
...
| Behavior | Evidence | Status |
|---|---|---|
| Semantic, temporal, lifecycle, deployment/plugin, and evidence mutations carry expected and label-withheld observed layer/code records. | `configs/cvm/fault_injection.yaml`; `artifacts/cvm/results/fault_injection.csv`; `tests/test_trace_diagnostics.py`; `tests/test_run_cvm_matrix.py`: `test_fault_row_derives_observation_from_mutated_trace`. | Covered: mutation and detection are separate; the detector is not passed the expected label. |
...
| Exact paired comparator counts are computed. | `classification.paired_comparison` in `artifacts/cvm/results/diagnostic_experiment.json`; `tests/test_trace_diagnostics.py`. | Covered descriptively: 15 WOD2Sim-only correct and 0 status-only-only correct. No population interval or significance test is reported for the designed suite. |
| Post-parse detector execution and a paired in-process adapter guard-path increment are measured without changing outputs. | `timing` and `adapter_guard_path_timing` in `artifacts/cvm/results/diagnostic_experiment.json`; `tests/test_trace_diagnostics.py`. | Covered for already-parsed in-memory detector execution and the adapter Drive path over 15 deterministic dependency-light sessions. The adapter timing includes input assembly, prediction, serialization, finite-output validation, reasoning parsing, and in-memory telemetry; gRPC transport, file I/O, simulator work, and human investigation are excluded. |
| Current-schema controls explicitly record finite serialized output; earlier telemetry cannot silently default missing evidence to valid. | `artifacts/cvm/inputs/diagnostic_protocol_sessions.jsonl`; `tests/test_trace_diagnostics.py`: `test_retained_protocol_sessions_are_separate_and_contract_clean`, `test_retained_external_v1_trace_exposes_missing_finite_output_evidence`. | Covered: 120/120 current drive records are explicitly finite, while the retained version-one external trace is classified as incomplete evidence. |
| One learned checkpoint completes a reactive AlpaSim lifecycle and a camera-validating control rejects the repeated frame. | `artifacts/external/alpasim_navsim_reactive_rollout/manifest.json`; `tests/test_alpasim_navsim_reactive_evidence.py`; `tests/test_aggregate_cvm.py`: `test_navsim_reactive_summary_revalidates_tracked_evidence`; `tests/test_validate_cvm_submission.py`: `test_reactive_artifact_integrity_matches_generated_summary`. | Covered for one camera-blind public-fixture configuration: 197/197 finite outputs and 198 live renderer requests are hash-validated. Repeated camera pixels and flat ground remain explicit exclusions. |
| Third-party-authored fault precision or simulator-rerun fault prevalence is evaluated. | No current generated artifact. | Gap: sessions and mutations are framework-authored deterministic cases, not external simulator fault reruns. |
...
- Detector and guard timers are software microbenchmarks, not end-to-end runtime or human time-to-diagnosis.
