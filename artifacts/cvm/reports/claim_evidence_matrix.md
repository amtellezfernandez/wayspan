...
| Temporal ablation evaluates full resampling versus naive/disabled resampling. | Temporal/Evidence | 18 paired rows with invalid-output and latency metrics. | `python scripts/run_cvm_matrix.py --config configs/cvm/temporal_ablation.yaml --output artifacts/cvm/results/temporal_ablation --resume --execute`. | `tests/test_run_cvm_matrix.py`; `tests/test_aggregate_cvm.py`. | `artifacts/cvm/results/temporal_ablation/runs.csv`. | Not supported: 18 rows blocked by missing direct-actor oracle proxy. | Results, Limitations. |
