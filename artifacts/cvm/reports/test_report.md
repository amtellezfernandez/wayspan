...
| `./.venv/bin/python scripts/validate_cvm_submission.py` | Passed, including metadata-backed title/author/affiliation/abstract checks, IEEE A4 source-layout checks, LaTeX log warnings, canonical-to-paper generated asset sync, claim-evidence-matrix count sync, embedded PDF font descriptors, manifest-level failure-attribution checks, summary-level attribution partition checks, and README/paper claim-boundary checks. |
| `make paper-verify PYTHON=./.venv/bin/python` | Passed: rebuilt 5-page root `wod2sim.pdf` and ran submission validation. |
| `make conformance PYTHON=./.venv/bin/python` | Passed: 259 passed, 14 skipped, 15 subtests passed. |
| `make demo PYTHON=./.venv/bin/python` | Passed: synthetic demo valid with `valid_claim_evidence=false`. |
| `make cvm-check PYTHON=./.venv/bin/python` | Passed: ruff clean, 259 passed, 14 skipped, 15 subtests passed, validation passed. |
...
| `./.venv/bin/python -m pytest -q` | Passed: 259 passed, 14 skipped, 15 subtests passed. |
...
| Selection | Result |
|---|---|
| `tests -k "semantic or route"` | 10 passed, 261 deselected. |
| `tests -k "temporal or resampl"` | 10 passed, 261 deselected, 15 subtests passed. |
| `tests -k "lifecycle or session"` | 10 passed, 261 deselected. |
| `tests -k "plugin or entry_point"` | 5 passed, 266 deselected. |
| `tests -k "deployment or readiness or launch"` | 20 passed, 251 deselected. |
| `tests -k "evidence or audit or benchmark"` | 19 passed, 252 deselected. |
| `tests -k "fault"` | 5 passed, 266 deselected. |
