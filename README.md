...
<p align="center">
  ...
  <a href="alpabridge.pdf">Paper</a> |
  <a href="docs/README.md">Documentation</a> |
  <a href="CITATION.cff">Citation</a>
</p>

AlpaBridge validates route meaning, policy-facing scene state, trajectory timing,
...
rule attributes a route-format failure correctly. AlpaBridge is a
contract-validation integration framework, not a new driving policy.

This repository has one canonical paper PDF: [`alpabridge.pdf`](alpabridge.pdf). The
...
Those directories are the reproducibility package for AlpaBridge; they are not a
separate project.
...
checkout, without redistributing them. AlpaBridge does not claim official Waymo
...
**Current AlpaSim boundary:** AlpaBridge targets a policy interface, not WOMD scenes.
...
| --- | --- | --- |
| Run a WOD-style trajectory policy on AlpaSim scenes | **Executed.** AlpaSim camera, ego motion, route, controller, and physics close the loop through AlpaBridge. | Start with [`route_following`](docs/getting-started.md#materialize-commands), then replace the policy behind the same declared input/output contract. |
...
| Evaluate a learned policy on representative WOMD scenarios | **Not implemented.** | Use [Waymax](https://github.com/waymo-research/waymax) with licensed WOMD data and a pinned learned checkpoint, then bind and test the declared AlpaBridge contracts. |
...
not missing AlpaSim data and not AlpaBridge's normal behavior. AlpaSim sends the
...
The rendered `alpasim-protocol-replay.mp4` comparison is not included in this
checkout; regenerate it with `scripts/run_alpasim_replay_demo.sh` from the
[validated manifest](artifacts/external/alpasim_protocol_replay/manifest.json)
and [reproduction notes](artifacts/external/alpasim_protocol_replay/README.md)
below.
...
| Does AlpaBridge prevent integration-invalid metrics from becoming policy evidence? | Yes. A defined status-only baseline accepts `15/15` completed metric-bearing command-only rows, while AlpaBridge rejects the same `15/15` as non-claim-valid route evidence. |
| Does the detector distinguish controlled faults from valid traces? | Yes, for the declared designed suite. On `15` single-fault mutations paired with `15` separately instantiated valid adapter sessions, AlpaBridge classifies `30/30`, localizes `15/15`, and flags `0/15` controls; the executable completion-and-metrics gate classifies `15/30` and detects no faults. These are exact descriptive counts, not a framework-superiority test. |
...
AlpaBridge's central release rule is separation between integration failure and
...
command-only rows, while AlpaBridge rejects those same `15/15` as non-claim-valid
...
records `197` AlpaBridge driver RPCs and `396` image events, and meets the driver
...
AlpaBridge gRPC services: route following and NAVSIM's hash-pinned official
...
scenario categories and `15` unclassified closed-loop scenes. AlpaBridge therefore
...
```mermaid
flowchart LR
    A[AlpaSim<br/>camera, ego motion, route] --> B[AlpaBridge input contract<br/>route + scene signal]
    B --> C{WOD-style policy}
    C --> D[Constant velocity / route following]
    C --> E[Published checkpoint adapters]
    C --> F[Direct actor planner]
    D --> G[AlpaBridge output contract<br/>resampling + headings]
    E --> G
    F --> G
...
**Figure 1.** AlpaBridge sits inside the closed loop. It translates AlpaSim state
...
alpabridge-reproduce \
  ...
  --evidence-dir /tmp/alpabridge/evidence \
  --json
```
...
reboot Windows before launching AlpaBridge. Installing a Linux NVIDIA kernel driver
...
After executing real batches, aggregate each driver with `alpabridge-batch-summary`
...
`make paper-verify` rebuilds the canonical [`alpabridge.pdf`](alpabridge.pdf) from
...
The demo writes a synthetic run directory under `demo/alpabridge-contract-demo`
...
[`alpabridge.pdf`](alpabridge.pdf) for the AlpaBridge paper.

## License And Disclaimer

AlpaBridge is released under the [BSD 3-Clause License](LICENSE). Packaged AlpaSim
