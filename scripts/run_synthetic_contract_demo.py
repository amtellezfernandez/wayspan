...
from wod2sim.cli.commands.support_bundle import build_report as build_support_bundle_report

DEFAULT_OUTPUT = ROOT / "demo" / "wod2sim-contract-demo"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a synthetic WOD2Sim contract demo with audit and support-bundle "
...
def _write_run_commands(run_dir: Path) -> None:
    ...
    _write_text(
        ...
        "\n".join(
            [
                "schema: wod2sim_synthetic_contract_demo_config_v1",
...
def _rollout_svg() -> str:
  ...
  <title id="title">Synthetic WOD2Sim Contract Demo</title>
...
def _print_human(summary: dict[str, Any]) -> None:
    print("WOD2Sim synthetic contract demo")
...
def main() -> int:
    ...
    try:
        ...
        print(f"wod2sim synthetic contract demo failed: {exc}", file=sys.stderr)
        return 1
    if args.json:
