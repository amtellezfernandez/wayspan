...
| 50-scene public 26.02 preset | Preset, cache-builder workflow, and one diagnostic local-probe summary are tracked. A claim-valid 50-scene closed-loop summary still requires rebuilding the local USDZ cache and executing the full stage on an x86_64 AlpaSim runner. |
